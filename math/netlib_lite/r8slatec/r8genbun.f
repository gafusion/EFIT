*DECK GENBUN
      SUBROUTINE r8genbun (NPEROD, N, MPEROD, M, A, B, C, IDIMY, Y,
     +   IERROR, W)
...
! idecl:  explicitize implicit INTEGER declarations:
      ...
      INTEGER n,mperod,m,idimy,ierror,nperod,i,mp1,iwba,iwbb,iwbc
      INTEGER iwb2,iwb3,iww1,iww2,iww3,iwd,iwtcos,iwp,k,j,mp,np
      INTEGER ipstor,irev,mh,mhm1,modd,mhpi,mhmi,nby2,mskip
!============
...
C***FIRST EXECUTABLE STATEMENT  GENBUN
  ...
  101 CONTINUE
      GO TO 104
  102 IF (A(1).NE.0._r8.OR. C(M).NE.0._r8) IERROR = 7
      GO TO 104
  103 IERROR = 6
  ...
  108 CALL r8poisp2 (M,N,W(IWBA),W(IWBB),W(IWBC),Y,IDIMY,W,W(IWB2),
     1             W(IWB3),W(IWW1),W(IWW2),W(IWW3),W(IWD),W(IWTCOS),
     2             W(IWP))
      GO TO 112
  109 CALL r8poisd2 (M,N,1,W(IWBA),W(IWBB),W(IWBC),Y,IDIMY,W,W(IWW1),
     1             W(IWD),W(IWTCOS),W(IWP))
      GO TO 112
  110 CALL r8poisn2 (M,N,1,2,W(IWBA),W(IWBB),W(IWBC),Y,IDIMY,W,W(IWB2),
     1             W(IWB3),W(IWW1),W(IWW2),W(IWW3),W(IWD),W(IWTCOS),
     2             W(IWP))
      GO TO 112
  111 CALL r8poisn2 (M,N,1,1,W(IWBA),W(IWBB),W(IWBC),Y,IDIMY,W,W(IWB2),
     1             W(IWB3),W(IWW1),W(IWW2),W(IWW3),W(IWD),W(IWTCOS),
     2             W(IWP))
  112 IPSTOR = W(IWW1)
      IREV = 2
  ...
  127 CONTINUE
      DO 132 J=1,N
         DO 128 I=1,MHM1
            MHMI = MH-I
            MHPI = MH+I
            W(MHMI) = .5_r8*(Y(MHPI,J)+Y(I,J))
            W(MHPI) = .5_r8*(Y(MHPI,J)-Y(I,J))
  128    CONTINUE
         W(MH) = .5_r8*Y(MH,J)
         GO TO (130,129),MODD
  129    W(M) = .5_r8*Y(M,J)
