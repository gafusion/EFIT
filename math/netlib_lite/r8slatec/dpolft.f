...
!============
     ...
     * SIGJM1,SIGPAS,TEMP,X(*),XM,Y(*),YP,W(*),W1,W11
     ...
     2  CO(4,3)/-13.086850_R8,-2.4648165_R8,-3.3846535_R8,-1.2973162_R8,
     3          -3.3381146_R8,-1.7812271_R8,-3.2578406_R8,-1.6589279_R8,
     4          -1.6282703_R8,-1.3152745_R8,-3.2640179_R8,-1.9829776_R8/
C***FIRST EXECUTABLE STATEMENT  DPOLFT
      ...
      IF (EPS .LT. 0.0_R8.AND.  M .EQ. MOP1) GO TO 30
      XM = M
      ETST = EPS*EPS*XM
      IF (W(1) .LT. 0.0_R8) GO TO 2
      DO 1 I = 1,M
        IF (W(I) .LE. 0.0_R8) GO TO 30
 1      CONTINUE
      GO TO 4
 2    DO 3 I = 1,M
 3      W(I) = 1.0_R8
 4    IF (EPS .GE. 0.0_R8) GO TO 8
C
C DETERMINE SIGNIFICANCE LEVEL INDEX TO BE USED IN STATISTICAL TEST FOR
C CHOOSING DEGREE OF POLYNOMIAL FIT
C
      IF (EPS .GT. (-.55_R8)) GO TO 5
      IDEGF = M - MAXDEG - 1
      KSIG = 1
      ...
      GO TO 8
 5    KSIG = 1
      IF (EPS .LT. (-.03_R8)) KSIG = 2
      IF (EPS .LT. (-.07_R8)) KSIG = 3
C
...
C
 23   IF (SIGJ .EQ. 0.0_R8) GO TO 29
      DEGF = M - J - 1
      DEN = (CO(4,KSIG)*DEGF + 1.0_R8)*DEGF
...
C
      IF(EPS .GE. 0.0_R8.OR.  NDEG .EQ. MAXDEG) GO TO 36
      NDER = 0
      DO 35 I = 1,M
