...
!
!  We replace the IMSL singular value decomposition routine LSVDF with
!  that from LINPACK here.  A brief description of each routine follows.
!-----------------------------------------------------------------------
!
!-----------------------------------------------------------------------
!  IMSL routine:
!-----------------------------------------------------------------------
!  
!  The singular value decomposition A = UQV**T of an m-by-n matrix A
...
!  where:
!
!  A   - REAL*8 M-by-N matrix.  On input, A contains the matrix to be 
...
!        replaced by the matrix product U**T * B.
!
...
!  Remarks:
!
!  1.  LSVDF computes the singular value decomposition of a REAL*8 M-by-N
!      matrix
!                        A = U * Q * V**T    where
!
...
!      output, B is replaced by U**T.
!
!  2.  The notation U**T and V**T represents U transpose and V transpose,
...
!                        X = V*Q**(+)*U**T*B.
!
!      U**T*B replaces B on output.  The solution X is obtained as follows:
...
!-----------------------------------------------------------------------
!
!  The singular value decomposition A = USV**T of an M-by-N matrix A can be
...
!  where
!
!  A    - REAL*8 M-by-N matrix A  [A(ADIM, N)].
!
...
!
!  JOB  - INTEGER*4 which indicates "how much" of SVD to compute.
!
!  INFO - INTEGER*4 array [INFO(N)]:
!         INFO = 0 implies SVD is found
!         INFO > 0 implies SVD is not completed
!
!  Remarks:
!
!      The variable JOB is a two-digit INTEGER*4 AB that prescribes "how
