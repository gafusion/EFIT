...
!      include 'ecomdu1.f90'
      ...
      character*10 nsingl(10),n1name,btcname &
                   ,nc79name,nc139name,ncname(3),niname(6)
...
!-- read in pointnames ...                                           --
!----------------------------------------------------------------------
      open(unit=60,file=table_dir(1:ltbdir)//'dprobe.dat', &
           status='old'                                )
      read(60,in3)
      close(unit=60)
!
...
!--------------------------------------------------------------------
      ...
      open(unit=60,file=input_dir(1:lindir)//'btcomp.dat', &
           status='old'                                )
31000 read (60,*,err=32000,end=32000) ibtcshot, btcname
...
35200   read (60,*,err=36000,end=36000) namedum,(dumcic(k),k=1,6)
        do i=1,magpol
         if (mpnam2(i).eq.namedum) then
           oldexp=expmpi(1,i)
           do j=1,np
...
!----------------------------------------------------------------------
   ...
   75   fccurt(j,i)=0.
        ...
        devfc(j,i) =devfc(j,i)*turnfc(i)        !-- EJS
   77 continue
      bitfc(i) =bitfc(i)*turnfc(i)
      fc_k(i)  =fc_k(i)*turnfc(i)	!-- EJS
   80 continue
      rnavfc = navfc
!----------------------------------------------------------------
...
!----------------------------------------------------------------------
	...
	npn = (tmax-tmin)/dtmin + 1.5           !-- changed 1 to 1.5 (EJS)
	npn = min0(npn,4000)
	npn = max0(npn,10)
!
        bitvl=0.0
...
20	if (mhigh .ge. nts) go to 30
	...
	go to 20
30	work(i)=val/(mhigh-mlow+1)
	if(kount .eq. 1) then   !-- calculate std dev based on raw data (EJS)
