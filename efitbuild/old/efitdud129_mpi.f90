...
! MPI >>>
     integer :: nargs, iargc, finfo, kerror
! MPI <<<
...
! MPI >>>
     ...
     if (rank == 0) then
       ...
       nw = 0
       nh = 0
       ! If user does not provide grid arguments, then we need to force error condition by explicitly setting nw and nh to zero
...
! MPI >>>
#if defined(USEMPI)
           ! WARNING : processes might not all enter conditional code section
           !if (nproc > 1) then
           !  ! NOTE : Want all processes to exit if any encounter error condition
...
!----------------------------------------------------------------------
           call fit(ks,kerror)
! MPI >>>
! NOTE : Recoverable error so do NOT terminate program
#if defined(USEMPI)
!           ! WARNING : processes might not all enter conditional code section
!           ! WARNING : dangerous to synchronize error codes since non-fatal error
!           !if (nproc > 1) then
!           !  ! NOTE : Want all processes to exit if any encounter error condition
!           !  ! ERROR : Means all processes must be allocated same amount of data
!           !  ! SYNC_ERROR
...
! MPI >>>
! NOTE : Serial version does NOT check error code because uses STOP command within CNTOUR subroutine, but
!        okay to leave error check code
        if (kerror == 1) return
! MPI <<<
        endif
...
!      equivalence (gridpf(1,1),gwork(1,1))
      ...
      ,ffbdry,kffbdry,ff2bdry,kff2bdry,errsil &
...
! jm.s
      ...
      geqdsk_ext,sign_ext,scalepp_ext,scaleffp_ext
...
!
      ...
      itimeu=0
      table_nam = table_dir
!      write(6,*) 'vasan 1 : ',table_dir,table_nam
      read (nin,in1)
...
!---------------------------------------------------------------------
      if (table_dir.ne.table_nam) then
!vas
!      print*,'file name : ', 'ec'//trim(ch1)// &  
!                         trim(ch2)//'.ddd' 
      open(unit=mcontr,status='old',form='unformatted', &
           file=table_dir(1:ltbdir)//'ec'//trim(ch1)//trim(ch2)//'.ddd')
!      write(6,*) 'vasan 2 : ',table_dir,table_nam
      read (mcontr) mw,mh
...
!----------------------------------------------------------------------
!vas
!vas      print*,'file name : ', 'ep'//trim(ch1)// &  
!vas                         trim(ch2)//'.ddd' 
      open(unit=mcontr,status='old',form='unformatted', &
           file=table_dir(1:ltbdir)//'ep'//trim(ch1)//trim(ch2)//'.ddd')
      read (mcontr) gsilpc
...
!-------------------------------------------------------------
!-- Read in fast-ion pressure                               --
!-------------------------------------------------------------
!      if (fastpre.ne.'none') then
!      endif
  216 continue
!
...
!
  325 continue
      if (jtime.le.1) then
        open(unit=80,status='old',file=table_dir(1:ltbdir)//'dprobe.dat')
        rsi(1)=-1.
        read (80,in3)
...
!vas
!vas      print*,'file name : ', 'rv'//trim(ch1)// &  
!vas                         trim(ch2)//'.ddd' 
      open(unit=nrsppc,status='old',form='unformatted', &
...
!vas
!vas      print*,'file name : ','ra'//trim(ch1)// &  
!vas                         trim(ch2)//'.ddd' 
        open(unit=nrsppc,status='old',form='unformatted', &
...
!vas
!vas      print*,'file name : ', 'ef'//trim(ch1)// &  
!vas                         trim(ch2)//'.ddd' 
        open(unit=mcontr,status='old',form='unformatted', &
...
!---------------------------------------------------------------------
!heng      if((kinputece.eq.1).and.(kfitece.gt.0)) then
!heng      ecetime=float(itime)
!heng      call hecedata(ishot,ecetime,tmehece,necein,feece0,teecein0,
!heng     & errorece0)
!heng      endif
!--------------------------------------------------------------------
...
!sri moved from here to after 1180
!-----------------------------------------------------------------------------
...
!----------------------------------------------------------------------
 ...
 1180 continue
!sri plasma response block is moved here for solove case
!-----------------------------------------------------------------------------
!--   set up plasma response                                                --
!-----------------------------------------------------------------------------
      do 72070 m=1,nbdry
...
72060   continue
72070 continue
!sri end of solove modification
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
 4000 continue
      if (nbdry.le.0.or.iconvr.ne.3) go to 4500
      erbmax=0.0
      erbave=0.0
      do 4100 i=1,nbdry
        xsibry=pbry(i)
...
!-----------------------------------------------------------------------
      ! UNUSED >>>
      !open(unit=911,file='errorm.log', status='replace')
      ! UNUSED <<<
      kerror=0
      jerror(jtime)=0
      nitera=0
...
!------------------------------------------------------------------
        if ((nitera.ge.kcallece).and.(kfitece.gt.0.0)) then
             nleft=abs(mxiter)-nitera
             if(nleft .gt. mtxece*nconstr) call setece(jtime)
        endif
        call green(nzero,jtime,nitera)
        if (kprfit.gt.0.and.iwantk.eq.ndokin) then
          call presur(jtime,nitera,kerror)
! MPI >>>
! NOTE : Error check missing in serial code
          if (kerror /= 0) then
            jerror(jtime) = 1
            return
          endif
...
! MPI <<<
           if (kwaitmse.ne.0 .and. i.ge.kwaitmse)  &
      	              call fixstark(jtime)
! MPI >>>
          ! NOTE : Check moved above before call to FIXSTARK
          !if (kerror.gt.0) then
          !  jerror(jtime)=1
          !  return
          !endif
...
! jm.s
      if (npsi_ext > 0) then
!        fpcurr = linear(ypsi,psin_ext,ffprim_ext,npsi_ext)
...
!
      nnn1=1
      iieerr=0
!EALW      write(*,*)'nnecein,mnow,nfit,nnn1,iieerr'
!EALW      write(*,*)nnecein,mnow,nfit,nnn1,iieerr
      call sdecm(arspfit,nnecein,mnow,nfit,brspfit,nnecein &
           ,nnn1,s,wk,iieerr)
!EALW      write(*,*) iieerr
...
3018    continue
         receo=rrr(iio)
!EALW           write(*,*) 'find receo, iio,rrr(iio),teeceo'
!EALW           write(*,*) iio,rrr(iio),teeceo
!EALW        write(*,*)'receo,zeceo'
!EALW        write(*,*)receo,zeceo
!--------------------------------------------------------------------
...
! MPI <<<
 ...
 81    continue
         open(unit=neqdsk,status='old', &
           file= input_dir(1:lindir)//snap_file         )
      endif
...
!
  ...
  102   continue
        ...
        enddo
        open(unit=neqdsk,status='old', &
             file=table_dir(1:ltbdir)//'fitweight.dat'         )
...
! MPI >>>
      ! FIXME - since only root process calls GETPTS and ALL processes already have error condition via MPI_BCAST then we do NOT need this code below
      !if (nproc > 1) then
      !  ! NOTE : Want all processes to exit if any encounter error condition
      !  call MPI_ALLREDUCE(istop,MPI_IN_PLACE,1,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,ierr)
      !endif
      if (istop.gt.0) then
        ! ONLY root process interfaces with terminal
        if (rank == 0) then
...
!
 1000 continue
!      write(6,*) 'vasan 3 : ',table_dir
!vas
!vas      print*,'file name : ','ec'//trim(ch1)// &  
!vas                         trim(ch2)//'.ddd' 
      open(unit=mcontr,status='old',form='unformatted', &
           file=table_dir(1:ltbdir)//'ec'//trim(ch1)//trim(ch2)//'.ddd')
      read (mcontr) mw,mh
...
!vasorg       allocate(gridpc(nwnh,nw),stat=iallocate_stat)
...
!
 ...
 4980 format (i5)
 5000 format (2e12.6)
! 5500 format (/,10x,'EFITD 129dx2 Version  ',2a5,/)
...
!      include 'ecomdu2.f90'
      ...
      !integer :: ktime_rank
      !if (nproc > 1) then
      !  fwtgam_mpi(:,rank+1) = fwtgam(:)
      !  !do i=dist_data_displs(rank+1)+1,dist_data_displs(rank+1)+dist_data(rank+1)
      !  !  fwtgam_mpi(:,i) = fwtgam(:)
      !  !enddo
      !endif
      ! TEMP <<<
   ...
   10 continue
	...
	enddo
      ...
      do 100 i=1,ktime
        ! MPI >>>
#if defined(USEMPI)
        !if (.not. i <= sum(dist_data(1:ktime_rank))) then
        !  ktime_rank = ktime_rank+1
        !endif
#endif
        ! MPI <<<
...
!
      ...
      open(unit=80,status='old', &
           file=table_dir(1:ltbdir)//'dprobe.dat' &
                                          )
      read (80,in3)
      close(unit=80)
...
! MPI <<<
! FIXME - Output garbled because multiple processes simultaneously writing to terminal
...
!-----------------------------------------------------------------
      if ((kfixro.eq.0).or.(kfixrece.eq.0)) call getecer(jtime,kerror)
! MPI >>>
#if defined(USEMPI)
      if (kerror /= 0) then
        return
...
!EALW      write(*,*)'setece, kece, kecebz, fwtecebz'
!EALW      write(*,*)kece,kecebz,fwtecebz
!------------------------------------------------------------------ 
      receoi(nitera)=receo
      do k=1,nece
...
40010    continue
 ...
 1105   siwant=simag-siwant*(simag-psibry)
...
!
      if (nbskip.le.0) then
        ...
        nbabs=max(nfound/mbdry+1,nbskip)
      endif
      jb=0
      iskip=nbabs-1
!-------------------------------------------------------------------
...
!**********************************************************************
      ...
      include 'eparmdud129.f90'
!vas      include 'modules2.f90' !to avoid clash with allocat...
      include 'modules1.f90'
!      include 'ecomdu1.f90'
!      include 'ecomdu2.f90'
      include 'env2d.inc'
      common/wwork1/xlims(5),ylims(5),limtrs,xlmins
!oct14      data ksetece/0/,kgeteceb/0/
!vasorg      data kgrid/1/,kwripre/0/,condin/1.0e-06/,data nbskip/2/
!oct14      data kgrid/1/,kwripre/0/,condin/1.0e-06/
!vasorg      data pi/3.1415926535897932/,tmu/2.0e-07/
!vasorg      data licalc/1/,dfsqe/0.0/,cbrsp/3.0e-5/
!oct14      data licalc/1/,cbrsp/3.0e-5/
!vasorg      data ircfact/0/,icalbet/1/,islpfc/0/,ksigma/0/
!oct14      data islpfc/0/,ksigma/0/
!vasorg      data ifcoil/1/,vertfb/0./,kdoqn/0/,iplots/1/
!oct14      data kdoqn/0/,iplots/1/
!vasorg      data iopen/0/,alphafp/0./,sigppb/1000./
!oct14      data alphafp/0./,sigppb/1000./
!oct14      data kframe/0/,rminvs/0/,rmaxvs/100./,zminvs/-100./,zmaxvs/100./
!oct14      data kskipvs/0/,vsdamp/0/,relbps/0.004/,zbound/0.0/,rbound/0.0/
!oct14      data icntour/0/,fwtxx/0.2/,fwtxxj/1./,n1coil/0/,nccoil/1/ &
!vasorg           ,errbry/1.0e-04/,nqiter/10/,cstabne/0.0/,cstabte/0.0/ &
!oct14           ,nqiter/10/,cstabne/0.0/,cstabte/0.0/ &
!vasorg           ,limvs/1/,ncstpp/1/,ncstfp/1/,psiwant/1.0/ &
!oct14           ,limvs/1/,ncstpp/1/,ncstfp/1/ &
!oct14           ,vzeroj(1)/0.0/,nqwant/0/,jbeta/1/,jli/2/,jwantm/3/ &
!oct14           ,sizeroj(1)/-1.0/,kzeroj/0/,nicoil/1/
!vasorg      data  fwtxxq/1./,fwtxxb/1./,fwtxli/1./,lring/0/,n_write/1/
!oct14      data  fwtxxq/1./,fwtxxb/1./,fwtxli/1./
!vasorg      data ibtcomp/1/,iaveus/0/
!oct14      data ibtcomp/1/
!vasorg      data cstabz/0.0e-13/,erbcom/1.0e-2/,errsil/0.03/
!oct14      data erbcom/1.0e-2/,errsil/0.03/
!vasorg      data dnmin/1.0/,kcaldia/0/,kcalpa/0/,kcgama/0/, &
!vasorg           kcomega/0/,msebkp/0/,msefitfun/1/
!oct14      data dnmin/1.0/,kcalpa/0/,kcgama/0/, &
!oct14           kcomega/0/,msebkp/0/,msefitfun/1/
!oct14      data kwaitmse/0/,dtmsefull/0.0/
!oct14      data mse_strict/0/,t_max_beam_off/0.0/
!oct14      data keecur/0/,ecurbd/0.0/,keefnc/0/,eetens/5.0/
!vasorg      data limid/33/,limfag/2/
!oct14      data limfag/2/
!oct14      data mfvers(1)/'06/10'/,mfvers(2)/'/2008'/
!vasorg      data nin/11/,nout/10/,ntty/5/,nrsppc/25/,nrspfc/26/ &
!vasorg           ,neqdsk/38/,nffile/40/,nsave/49/ &
!vasorg           ,nttyo/6/,ivnit/35/
!oct14      data kbound/0/,iacoil/0/,fwacoil/1*0./
!vasorg      data saimin/60.0000/,saicon/60.0/,errmin/0.010/,elomin/0.90/
!oct14      data saimin/60.0000/,saicon/60.0/,elomin/0.90/
!vasorg      data gain/.22/,gainp/0.75/,ishiftz/0/,vfeed/.false./,ioffz/7/, &
!vasorg           ioffr/-7/,fitdelz/.false./,ndelzon/999/,relaxdz/1.000/, &
!vasorg           stabdz/-1.e-4/,scalesir/1.e-3/,scaledz/1.e-03/, &
!vasorg           oldccomp/.true./,oldcomp/.false./,ifitdelz/1/
!oct14      data fitdelz/.false./,ndelzon/999/,relaxdz/1.000/, &
!oct14           stabdz/-1.e-4/,scaledz/1.e-03/, &
!oct14           oldccomp/.true./,oldcomp/.false./,ifitdelz/1/

!vasorg      data fitsiref/.false./,writepc/.false./,scalea/.false./
!oct14      data writepc/.false./
!vasorg      data kdofit/0/,kthkcrv/0/,errdelz/0.06/
!oct14      data kdofit/0/,errdelz/0.06/
!vasorg      data imagsigma/0/, relax/1.00/,errorm/10./, errmag/1.0e-3/ &
!vasorg           , errmagb/1.e-2/
!oct14      data imagsigma/0/, errmag/1.0e-3/ &
!oct14           , errmagb/1.e-2/
!vasorg      data limtrs/5/,rexpan/0.010/,znose/-1.276/
      data limtrs/5/
!vasorg      data fco2ne/1.0/,cupdown/-100000./,kccoils/0/
!oct14      data fco2ne/1.0/,kccoils/0/
!oct14      data kvtor/0/,rvtor/1.70/,preswb/0.0/,wcurbd/0.0/,betapw0/0.0/ &
!oct14           ,kwwcur/2/,kplotp/1/,ndokin/1/,kdomse/0/,kdovt/0/,nsplot/4/ &
!oct14           ,kdopre/0/,kdoece/0/,psiecn/0.0/,dpsiecn/0.0/
!vasorg      data n111/1/,n222/2/,x111/1.0/,x000/0.0/,n333/3/,n444/4/,n555/5/ &
!vasorg           ,n666/6/,kersil/2/  
!oct14      data kersil/2/  
      data iunit/35/, m_write/1/, m_read/1/
      data out2d /'curve2d.dat'/, out2d_bin/'curve2d_bin.dat'/
!vasorg      data kedgep/0/,pe_width/0.02/,pe_psin/0.98/,pedge/0.0/, &
!vasorg           kedgef/0/,fe_width/0.02/,fe_psin/0.98/,f2edge/0.0/
!oct14      data fitzts/'no'/
!
      end
! -- Qilong Ren
! 
! Subroutine for writing K file 
!
      subroutine write_K(jtime,kerror)
      include 'eparmdud129.f90'
      include 'modules2.f90'
      include 'modules1.f90'
!      include 'ecomdu1.f90'
...
12928    continue
 ...
 3025 open(unit=nout,status='old', &
           ...
           ,err=3020)
!
!c -- recover the value of table_dir for mode 3 or 7
      else
      if(ishot.ge.112000.and.jtime.le.1) then
      ltbdir = ltbdir-7
...
! MPI >>>
#if defined(USEMPI)
! ERROR - Existed in original MPI code but means ONLY root process writes k-files
      !if (nproc > 1 .and. rank > 0) then
      !  return
      !endif
#endif
! MPI <<<
