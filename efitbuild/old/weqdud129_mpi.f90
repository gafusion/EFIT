...
!      include 'ecomdu2.f90'
      ...
      namelist/in1/ishot,itime,betap0,rzero,qenp,enp,emp,plasma, &
           ...
           iconvr,ibunmn,brsp,kffcur,kppcur,pcurbd,fcurbd
      namelist/basis/kppfnc,kppknt,ppknt,pptens, &
                   kfffnc,kffknt,ffknt,fftens, &
                   kwwfnc,kwwknt,wwknt,wwtens, &
                   ppbdry,pp2bdry,kppbdry,kpp2bdry, &
      ...
      character*10 case(6)
      ! BUGFIX : need to make dynamically allocated local arrays as large as possible
      !          in order to accommodate cases where NPRESS > NW and NH
      integer :: parameter
      parameter (pltnw=1025)
      real*8,dimension(:),allocatable :: workk,dmion,bworm,cworm, &
             dworm
      dimension tgamma(nstark),sgamma(nstark),rrrgam(nstark) &
